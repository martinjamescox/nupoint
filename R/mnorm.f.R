...
mnorm.f <- function(z,
  ...
  norMixObj=nor1mix::norMix(mu=pars[seq(1,(n*3)-2,3)], 
                   sigma = pars[seq(2,(n*3)-1,3)]**2, w = wts) #creates a normal mixture object.
  
  den=nor1mix::dnorMix(x=z, obj=norMixObj)/diff(nor1mix::pnorMix(q=c(z.lim[1],zMax), obj=norMixObj))
